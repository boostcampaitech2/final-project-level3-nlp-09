...
def get_config():
    ...
    parser.add_argument('--train_path', type= str, default= '/opt/ml/sentiment_analysis/data/train.csv',
                        help='train csv path (default: /opt/ml/sentiment_analysis/data/train.csv')
    parser.add_argument('--save_dir', type=str, default = './best_model', 
                        help='model save dir path (default : ./best_model)')
    parser.add_argument('--wandb_path', type= str, default= 'roberta_default',
                        help='wandb graph, save_dir basic path (default: roberta_default')    
...
def train(args):
    ...
    all_label= all_dataset["감정_소분류"].values
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(all_dataset, all_label)):
        run= wandb.init(project= 'sentiment', entity= 'quarter100', name= args.wandb_path)
        print(f'fold: {fold} start!')
        train_dataset= all_dataset.iloc[train_idx]
        val_dataset= all_dataset.iloc[val_idx]
        
        train_label= preprocess.label_to_num(train_dataset['감정_소분류'].values)
        val_label= preprocess.label_to_num(val_dataset['감정_소분류'].values)
